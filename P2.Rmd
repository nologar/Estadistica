...
cat('\nResultados de los criterios AIC y BIC para el modelo simple aplicado a profundidad son: \nAIC:', AIC_prof_simple,'\nBIC:', BIC_prof_simple)
...
cat('\nResultados de los criterios AIC y BIC para el modelo ponderado aplicado a profundidad son: \nAIC:', AIC_prof_pond,'\nBIC:', BIC_prof_pond)

densidadobtenida <- densidadmg( data$depth, resultado_prof_pond$par[1], resultado_prof_pond$par[2], resultado_prof_pond$par[3], resultado_prof_pond$par[4], resultado_prof_pond$par[5] )

ggplot( data = data, aes(x = depth))+
  geom_histogram( aes(y = ..density..) )+
  geom_line( aes( y = densidadobtenida), linewidth = 1)
  

```
En este caso vemos que el modelo simple nos da mejores resultados tanto para el criterio AIC como para el BIC tanto en el caso de la profundidad como para la latitud.
...
Los intervalos de confianza nos permiten estimar el rango en el que se encuentra la media verdadera de las variables analizadas (longitud, latitud y profundidad) con un nivel de confianza del 95 %. En este caso, se observa que los intervalos de confianza de cada variable no se solapan entre los dos grupos (Tonga trench y Plate junction). Esto confirma que existen diferencias estadísticamente significativas entre ambos conjuntos de eventos.
