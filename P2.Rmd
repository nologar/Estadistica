...
```
Vemos que los modelos són prácticamente idénticos, y esto lo podemos esto es porque el modelo de mezcla ha considerado solo una gaussiana ( el parámetro peso es 0.99)
```{r}
resultado_lat_pond$par
```
y por esto tiene sentido que el modelo simple, que tiene menos parámetros salga beneficiado del criterio AIC/BIC.

Y a continuación de la profundidad:

```{r ej5.2,echo=FALSE, warning = FALSE}
...
```{r ej6, message=FALSE, warning=FALSE,echo=FALSE}
